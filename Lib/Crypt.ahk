...
class Crypt
{
	class Encrypt
	{
		...
		StrEncrypt(ByRef string,password,CryptAlg = 1, HashAlg = 1)
		{
		...
		StrDecrypt(ByRef dec_out, EncryptedHash,password,CryptAlg = 1, HashAlg = 1, Manual=false)
		{
			if !EncryptedHash
				return ""
			if !len := b64Decode( EncryptedHash, encr_Buf )
			...
			{
				memmove(&dec_out, &encr_Buf, sLen)
				memzero(&encr_Buf, sLen)
				return sLen
			}
			else
				return ""
...
memmove(ByRef dst, ByRef src, ByRef len) {
	return DllCall("Ntdll.dll\RtlMoveMemory", "Ptr", dst, "Ptr", src, "UPtr", len)
}

memzero(ByRef dst, ByRef len) {
	return DllCall("Ntdll.dll\RtlZeroMemory", "Ptr", dst, "UPtr", len)
}